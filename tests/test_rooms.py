...
async def test_creator_is_automatically_admin(client):
    ...
    create_response = await client.post("/rooms", json={
        "name": "adminroom",
        "is_private": False,
    }, headers=auth_header)
    room_id = create_response.json()["id"]

    members_response = await client.get(f"/rooms/{room_id}/members", headers=auth_header)
...
async def test_member_can_send_and_list_messages(client):
    # Owner creates room
    auth_header = await _register_and_login(client)
    room_response = await client.post("/rooms", json={
        "name": "adminroom",
        "is_private": False,
    }, headers=auth_header)
